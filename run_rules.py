...
GROUND_TRUTH = {
    'Narayanan': dict(ug=1, grad=1, ms=0, phd=1, grants=0, ch_co=6, cp=4, journal=1),
    'Qian':      dict(ug=0, grad=1, ms=0, phd=1, grants=1, ch_co=16, cp=15, journal=8),
    'Palermo':   dict(ug=1, grad=3, ms=1, phd=3, grants=5, ch_co=13, cp=14, journal=11),
    'Hu':        dict(ug=2, grad=2, ms=1, phd=1, grants=4, ch_co=10, cp=5,  journal=2),
    'Duffield':  dict(ug=0, grad=1, ms=3, phd=0, grants=5, ch_co=2,  cp=6,  journal=9),
}

def compare_to_gt(result):
    name = result['last_name']
    if name not in GROUND_TRUTH:
        return
    gt = GROUND_TRUTH[name]
    fields = ['ug','grad','ms','phd','grants','ch_co','cp','journal']
    matches = sum(1 for f in fields if result[f] == gt[f])
    total = len(fields)
    print(f"\n  GT comparison ({name}): {matches}/{total}")
    print(f"  {'Field':<10} {'Got':>5} {'GT':>5} {'':>6}")
    for f in fields:
        ok = "✓" if result[f] == gt[f] else "✗"
        print(f"  {f:<10} {result[f]:>5} {gt[f]:>5}  {ok}")
    return matches


# ── CLI ────────────────────────────────────────────────────────────────────────

KNOWN_FACULTY = ['Narayanan', 'Qian', 'Palermo', 'Hu', 'Duffield']
...
def main():
    ...
    if results:
        ...
        if total_cells:
            print(f"\n{'='*60}")
            print(f"Overall: {total_match}/{total_cells} cells match ground truth")
